  ...
  <div class="comment space-bottom">
    <div class="goods-box">
      <div class="goods-item">
        ...
        <div class="comment-again" v-if="commentAgain && commentAgain.content"><span>【追评】</span>{{ commentAgain.content }}</div>
      </div>
      <bl-upload ref="upload"></bl-upload>
      <div class="goods-item" v-if="!(commentAgain && commentAgain.content)">
    ...
    <div class="comment-bottom flex-space flex-m">
      ...
      <div class="comment-right">
        <bl-button class="middle" @click="commentAfter" :disabled="messageBoole">发布晒单</bl-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    commentAfter() {
      ...
      if (this.commentAgain && this.commentAgain.content) {
        if (this.$refs.upload.upload.length > 0) {
          this.commentPic()
        } else {
          this.$toast('追评内容不能为空')
        }
    ...
    commentPic() {
      let picsList = []
      let refUpload = this.$refs.upload.upload
      if (refUpload.length > 0) {
        for (let i = 0; i < refUpload.length; i++) {
          picsList.push(refUpload[i].url)
        }
      }
      api.addCommentPic({
        commentId: this.product.id,
        content: this.$refs.comment ? this.$refs.comment.message : '',
        picsList: picsList,
        cephList: picsList
      }).then(res => {
        if (res.body.obj) {
